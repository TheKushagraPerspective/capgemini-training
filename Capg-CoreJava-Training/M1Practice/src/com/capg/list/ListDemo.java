package com.capg.list;

import java.util.ArrayList;
import java.util.Collections;
import java.util.Iterator;
import java.util.LinkedList;

public class ListDemo {
	public static void main(String []args) {
		
		System.out.println("----- ARRAYLIST DEMO -----");

        ArrayList<Integer> list = new ArrayList<>();
        list.add(10);
        list.add(30);
        list.add(20);
        
        list.add(1 , 15);
        System.out.println("List: " + list);
        
        System.out.println("Element at index 2: " + list.get(2));
        
        list.set(2 , 25);
        System.out.println("After set: " + list);
        
        list.remove(0);
        System.out.println("After remove index: " + list);
        
        list.remove(Integer.valueOf(25));
        System.out.println("After remove object: " + list);
        
     // contains()
        System.out.println("Contains 30? " + list.contains(30));

        // size()
        System.out.println("Size: " + list.size());

        // isEmpty()
        System.out.println("Is Empty? " + list.isEmpty());
        
        list.add(10);
        list.add(null);
        System.out.println("With duplicates & null: " + list);
        
        System.out.println("For-each loop:");
        for(Integer i : list) {
        	System.out.print(i + " ");
        }
        System.out.println();
        
        System.out.println("Iterator:");
        Iterator<Integer> itr = list.iterator();
        
        while(itr.hasNext()) {
        	System.out.print(itr.next() + " ");
        }
        System.out.println();
        
        
        list.remove(null);
        
        Collections.sort(list);
        System.out.println(list);
        
        Collections.sort(list , Collections.reverseOrder());
        System.out.println(list);
        
        list.sort((a , b) -> a - b);
        System.out.println(list);
        
        list.clear();
        System.out.println("After clear: " + list);
        
		System.out.println("\n----- LINKEDLIST DEMO -----");

        LinkedList<Integer> ll = new LinkedList<>();
        ll.add(10);
        ll.add(30);
        ll.add(20);
        System.out.println(ll);
        
        ll.addFirst(5);
        ll.addLast(35);
        System.out.println(ll);
        
     // getFirst() & getLast()
        System.out.println("First: " + ll.getFirst());
        System.out.println("Last: " + ll.getLast());

        // removeFirst() & removeLast()
        ll.removeFirst();
        ll.removeLast();

        System.out.println("After removing first & last: " + ll);
        
     // peek(), poll()
        System.out.println("Peek: " + ll.peek());
        System.out.println("Poll: " + ll.poll());

        System.out.println("After poll: " + ll);
        
     // add again for sorting
        ll.add(50);
        ll.add(15);
        
     // Sorting LinkedList
        Collections.sort(ll);
        System.out.println("LL Ascending: " + ll);

        Collections.sort(ll, Collections.reverseOrder());
        System.out.println("LL Descending: " + ll);
	}
}
